...
def sensor():
    ...
    while True:
        ...
        if newdata[0:6] == "$GPRMC":
            ...
            hiz = speed_knot * 1.852  # 1 knot = 1.852 km/h
            print(f'Hız: {hiz:.2f} km/h')
            
            if hiz<35:
                hizson=10
                hiz=10
                print(hizson)
            elif hiz>=35 and hiz<45:
    ...
    #             print("ses var")
        ...
        toplam=(riskMesafe*(math.log(riskMesafe,2)/2))+(hizson*(math.log(hiz*2,2)/7))+(ivmeOlcDeger*(math.sin(math.radians(ivmex*100)))*2)+(ivmeOlcDeger2*(math.sin(math.radians(ivmey*100)))*2)+(yagmurRisk*1)+(isikRisk*1)+(sesRisk*1)
        toplam=float(toplam)
        if toplam >=60:
            print("Sonuç: ",toplam," Uyarı Risk Yüksek")
        elif toplam < 60:
            print("Sonuç: ",toplam)
        if toplam<=12.5:
            pixels1 = neopixel.NeoPixel(board.D12, 55, brightness=0.05)
...
thread1 = threading.Thread(target=gozAlgilama)
